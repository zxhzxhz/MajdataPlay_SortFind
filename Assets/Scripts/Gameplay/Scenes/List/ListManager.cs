...
namespace MajdataPlay.List
{
    public class ListManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            var selectsfx = AudioManager.GetSFX("bgm_select.mp3");
            if (!selectsfx.IsPlaying)
            {
                AudioManager.PlaySFX("bgm_select.mp3", true);
                AudioManager.PlaySFX("SelectSong.wav");
            }
        }
            ...
            if (!e.IsButton&&e.IsDown)
            {
                switch (e.Type)
                {
                    ...
                    case SensorArea.E7:
                        ...
                        AudioManager.PlaySFX(list[UnityEngine.Random.Range(0, list.Length)]);
                        ...
                        case SensorArea.A4:
                            if (_coverListDisplayer.IsDirList)
                            {
                                if (_coverListDisplayer.SelectedCollection.Type == ChartStorageType.Dan)
                                {
                                    ...
                                    AudioManager.StopSFX("bgm_select.mp3");
        ...
        async UniTaskVoid PracticeTimer()
        {
            while (_isPressed)
            {
                ...
                if(_pressTime > 1f)
                {
                    AudioManager.PlaySFX("bgm_explosion.mp3");
                    break;
                }
            }
        }
