...
namespace MajdataPlay.IO
{
    public class LightManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            _isEnabled = MajEnv.UserSetting.Misc.OutputDevice.Led.Enable;
            for (var i = 0; i < 8; i++)
            {
                _ledDevices[i] = new()
                {
            ...
            var comPort = MajEnv.UserSetting.Misc.OutputDevice.Led.COMPort;
            var comPortStr = $"COM{comPort}";
            try
            {
                _serial = new SerialPort(comPortStr, MajEnv.UserSetting.Misc.OutputDevice.Led.BaudRate);
        ...
        async void UpdateLedDeviceAsync()
        {
            ...
            if (_useDummy || !MajEnv.UserSetting.Misc.OutputDevice.Led.Enable)
                return;
        ...
        async Task UpdateInternalLedManagerAsync()
        {
            ...
            var token = MajEnv.GlobalCT;
            var refreshRate = TimeSpan.FromMilliseconds(MajEnv.UserSetting.Misc.OutputDevice.Led.RefreshRateMs);
