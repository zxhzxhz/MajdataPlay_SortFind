...
namespace MajdataPlay.Result
{
    public partial class ResultScreenManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            AudioManager.PlaySFX("bgm_result.mp3", true);
        ...
        async UniTask PlayVoice(double dxacc, ISongDetail song)
        {
            if (dxacc >= 97)
            {
                AudioManager.PlaySFX("Rank.wav");
                await UniTask.WaitForSeconds(1);
            }
            if (dxacc >= 100.5f)
            {
                AudioManager.PlaySFX("SSS+.wav");
                rank.text = "SSS+";
            }
            else if (dxacc >= 100f)
            {
            ...
            if (dxacc > 97)
            {
                ...
                AudioManager.PlaySFX(list[Random.Range(0, list.Length)]);
                await UniTask.WaitForSeconds(3);
                if (song is OnlineSongDetail)
                {
                    AudioManager.PlaySFX("dian_zan.wav");
                }
            }
            else
            {
                ...
                switch (e.Type)
                {
                    case SensorArea.A4:
                        ...
                        if (_gameInfo.IsDanMode)
                        {
                            ...
                            {
                                InputManager.UnbindAnyArea(OnAreaDown);
                                AudioManager.StopSFX("bgm_result.mp3");
