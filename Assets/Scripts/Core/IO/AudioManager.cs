...
namespace MajdataPlay.IO
{
    ...
    {
        readonly static string SFXFilePath = MajEnv.AssetsPath + "/SFX/";
        readonly static string VoiceFilePath = MajEnv.AssetsPath + "/Voice/";
        static string[] SFXFileNames = new string[0];
        static string[] VoiceFileNames = new string [0];
        private static List<AudioSampleWrap> SFXSamples = new();
        ...
        static AudioManager()
        {
            var isExclusiveRequest = MajEnv.UserSetting.Audio.WasapiExclusive;
            var backend = MajEnv.UserSetting.Audio.Backend;
            var sampleRate = MajEnv.UserSetting.Audio.Samplerate;
            var deviceIndex = MajEnv.UserSetting.Audio.AsioDeviceIndex;
        ...
        static void InitSFXSample(string[] fileNameList,string rootPath)
        {
        ...
        // static void OnAnyAreaDown(object sender, InputEventArgs e)
        // {
        ...
        public static void ReadVolumeFromSettings()
        {
            var volume = MajEnv.UserSetting.Audio.Volume;
            foreach(var sample in SFXSamples)
            {
        ...
        public static AudioSampleWrap LoadMusic(string path, bool speedChange = false)
        {
            var backend = MajEnv.UserSetting.Audio.Backend;
            if (File.Exists(path))
            {
        ...
        public static AudioSampleWrap LoadMusicFromUri(Uri uri)
        {
            var backend = MajEnv.UserSetting.Audio.Backend;
            switch (backend)
            {
        ...
        public static async UniTask<AudioSampleWrap> LoadMusicAsync(string path, bool speedChange = false)
        {
        ...
        public static async UniTask<AudioSampleWrap> LoadMusicFromUriAsync(Uri uri)
        {
            await UniTask.SwitchToThreadPool();
            var backend = MajEnv.UserSetting.Audio.Backend;
            switch (backend)
        ...
        public static void PlaySFX(string name, bool isLoop = false)
        {
        ...
        public static AudioSampleWrap GetSFX(string name)
        {
            var psp = SFXSamples.FirstOrDefault(o => o.Name == name);
            if (psp is not null)
            {
        ...
        public static void OpenAsioPannel()
        {
            if(MajEnv.UserSetting.Audio.Backend == SoundBackendType.Asio)
            {
